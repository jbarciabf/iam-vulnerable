# Privesc Path 7: actAs + Cloud Build
#
...
resource "google_service_account" "privesc7_actas_cloudbuild" {
...
resource "google_service_account_iam_member" "privesc7_actas" {
  service_account_id = google_service_account.high_priv.name
  role               = "roles/iam.serviceAccountUser"
  member             = "serviceAccount:${google_service_account.privesc7_actas_cloudbuild.email}"
}

# Grant Cloud Build editor permissions
resource "google_project_iam_member" "privesc7_cloudbuild" {
  project = var.project_id
...
resource "google_service_account_iam_member" "privesc7_impersonate" {
  service_account_id = google_service_account.privesc7_actas_cloudbuild.name
