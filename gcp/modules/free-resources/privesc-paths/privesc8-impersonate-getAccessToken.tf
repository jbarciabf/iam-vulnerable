# Privesc Path 8: Direct Impersonation via getAccessToken
#
...
resource "google_service_account" "privesc8_get_access_token" {
...
resource "google_service_account_iam_member" "privesc8_token_creator" {
  service_account_id = google_service_account.high_priv.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "serviceAccount:${google_service_account.privesc8_get_access_token.email}"
}

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "privesc8_impersonate" {
  service_account_id = google_service_account.privesc8_get_access_token.name
  role               = "roles/iam.serviceAccountTokenCreator"
