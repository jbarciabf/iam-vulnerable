# Privesc Path 5: actAs + Cloud Function Deployment
#
...
resource "google_service_account" "privesc5_actas_function" {
...
resource "google_service_account_iam_member" "privesc5_actas" {
  service_account_id = google_service_account.high_priv.name
  role               = "roles/iam.serviceAccountUser"
  member             = "serviceAccount:${google_service_account.privesc5_actas_function.email}"
}

# Grant Cloud Functions developer permissions
resource "google_project_iam_member" "privesc5_functions" {
  project = var.project_id
...
resource "google_service_account_iam_member" "privesc5_impersonate" {
  service_account_id = google_service_account.privesc5_actas_function.name
