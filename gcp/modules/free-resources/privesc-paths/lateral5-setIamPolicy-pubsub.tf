# Lateral Movement Path 5: setIamPolicy on Pub/Sub Resources
#
...
resource "google_service_account" "lateral5_pubsub_iam" {
  account_id   = "lateral5-pubsub-iam"
  display_name = "Lateral5 - Pub/Sub IAM"
  description  = "Can access messages via Pub/Sub IAM modification"
...
resource "google_pubsub_topic_iam_member" "lateral5_topic_admin" {
  topic  = google_pubsub_topic.target_topic.name
  role   = "roles/pubsub.admin"
  member = "serviceAccount:${google_service_account.lateral5_pubsub_iam.email}"
}

# Grant admin ONLY on the target subscription
resource "google_pubsub_subscription_iam_member" "lateral5_subscription_admin" {
...
resource "google_service_account_iam_member" "lateral5_impersonate" {
  service_account_id = google_service_account.lateral5_pubsub_iam.name
