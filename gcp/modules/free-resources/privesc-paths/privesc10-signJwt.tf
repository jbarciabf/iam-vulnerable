...
resource "google_service_account" "privesc10_sign_jwt" {
...
resource "google_project_iam_member" "privesc10_role" {
  project = var.project_id
  role    = google_project_iam_custom_role.sign_jwt.id
  member  = "serviceAccount:${google_service_account.privesc10_sign_jwt.email}"
}

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "privesc10_impersonate" {
  service_account_id = google_service_account.privesc10_sign_jwt.name
