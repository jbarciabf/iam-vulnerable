# Privesc Path 16: Explicit Deny Bypass via Service Account Chaining
#
...
resource "google_service_account" "privesc16_deny_bypass" {
...
resource "google_service_account_iam_member" "privesc16_impersonate_medium" {
  service_account_id = google_service_account.medium_priv.name
  role               = "roles/iam.serviceAccountTokenCreator"
  member             = "serviceAccount:${google_service_account.privesc16_deny_bypass.email}"
}

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "privesc16_impersonate" {
  service_account_id = google_service_account.privesc16_deny_bypass.name
  role               = "roles/iam.serviceAccountTokenCreator"
