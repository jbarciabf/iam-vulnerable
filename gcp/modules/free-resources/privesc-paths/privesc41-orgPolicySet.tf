# Privesc Path 41: Organization Policy Set
#
...
#
# NOTE: This path is DISABLED by default (enable_privesc41 = false) because
...
#
# Enable with: enable_privesc41 = true (requires gcp_organization_id)

resource "google_service_account" "privesc41_org_policy" {
  count = var.enable_privesc41 ? 1 : 0

  account_id   = "${var.resource_prefix}41-org-policy"
  display_name = "Privesc41 - orgpolicy.policy.set"
...
resource "google_project_iam_member" "privesc41_org_policy" {
  count = var.enable_privesc41 ? 1 : 0

  project = var.project_id
  role    = "roles/orgpolicy.policyAdmin"
  member  = "serviceAccount:${google_service_account.privesc41_org_policy[0].email}"
}
...
resource "google_service_account_iam_member" "privesc41_impersonate" {
  ...
  service_account_id = google_service_account.privesc41_org_policy[0].name
