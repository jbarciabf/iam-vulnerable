# Privesc Path 11: Update Custom Role
#
...
resource "google_service_account" "privesc11_update_role" {
...
resource "google_project_iam_member" "privesc11_modifiable" {
  project = var.project_id
  role    = google_project_iam_custom_role.modifiable_role.id
  member  = "serviceAccount:${google_service_account.privesc11_update_role.email}"
}

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "privesc11_impersonate" {
  service_account_id = google_service_account.privesc11_update_role.name
