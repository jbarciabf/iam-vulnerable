# Privesc Path 29: Cloud Composer Update (Hijack Existing Environment)
#
...
#
# NOTE: Unlike Path 28, this does NOT require composer.environments.create.
...
#
# DISABLED BY DEFAULT: Set enable_privesc29 = true to enable
#
...
resource "google_service_account" "privesc29_composer_update" {
  count = var.enable_privesc29 ? 1 : 0

  account_id   = "${var.resource_prefix}29-composer-update"
  display_name = "Privesc29 - Composer Update"
...
resource "google_project_iam_custom_role" "privesc29_composer_update" {
  count = var.enable_privesc29 ? 1 : 0

  role_id     = "${var.resource_prefix}_29_composer_update"
  title       = "Privesc29 - Composer Update Only"
...
resource "google_project_iam_member" "privesc29_role" {
  ...
  project = var.project_id
  role    = google_project_iam_custom_role.privesc29_composer_update[0].id
  member  = "serviceAccount:${google_service_account.privesc29_composer_update[0].email}"
}
...
resource "google_service_account_iam_member" "privesc29_impersonate" {
  ...
  service_account_id = google_service_account.privesc29_composer_update[0].name
