# Privesc Path 16: actAs + Cloud Function Deployment
#
...
resource "google_service_account" "privesc16_actas_function" {
  account_id   = "${var.resource_prefix}16-actas-function"
  display_name = "Privesc16 - actAs + Cloud Function"
...
resource "google_service_account_iam_member" "privesc16_actas" {
  service_account_id = google_service_account.high_priv.name
  role               = "roles/iam.serviceAccountUser"
  member             = "serviceAccount:${google_service_account.privesc16_actas_function.email}"
}

# Custom role with minimal Cloud Functions permissions
resource "google_project_iam_custom_role" "privesc16_functions" {
  role_id     = "${var.resource_prefix}_16_cloudfunctions"
  title       = "Privesc16 - Cloud Functions Deploy"
...
resource "google_project_iam_member" "privesc16_functions" {
  project = var.project_id
  role    = google_project_iam_custom_role.privesc16_functions.id
  member  = "serviceAccount:${google_service_account.privesc16_actas_function.email}"
}

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "privesc16_impersonate" {
  service_account_id = google_service_account.privesc16_actas_function.name
...
resource "google_service_account_iam_member" "privesc16_actas_default_compute" {
