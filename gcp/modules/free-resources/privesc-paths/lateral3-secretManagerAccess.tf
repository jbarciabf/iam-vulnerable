# Lateral Movement Path 3: Secret Manager Access
#
# CATEGORY: Data Exfiltration / Credential Theft (NOT privilege escalation)
#
...
resource "google_service_account" "lateral3_secret_access" {
  account_id   = "lateral3-secret-access"
  display_name = "Lateral3 - Secret Manager"
...
resource "google_secret_manager_secret_iam_member" "lateral3_secret_accessor" {
  secret_id = google_secret_manager_secret.target_secret.id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.lateral3_secret_access.email}"
}

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "lateral3_impersonate" {
  service_account_id = google_service_account.lateral3_secret_access.name
