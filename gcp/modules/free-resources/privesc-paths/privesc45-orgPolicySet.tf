# Privesc Path 45: Organization Policy Set
#
...
#
# NOTE: This path is DISABLED by default (enable_privesc45 = false) because
...
#
# Enable with: enable_privesc45 = true (requires gcp_organization_id)

resource "google_service_account" "privesc45_org_policy" {
  count = var.enable_privesc45 ? 1 : 0

  account_id   = "${var.resource_prefix}45-org-policy"
  display_name = "Privesc45 - orgpolicy.policy.set"
...
resource "google_project_iam_member" "privesc45_org_policy" {
  count = var.enable_privesc45 ? 1 : 0

  project = var.project_id
  role    = "roles/orgpolicy.policyAdmin"
  member  = "serviceAccount:${google_service_account.privesc45_org_policy[0].email}"
}
...
resource "google_service_account_iam_member" "privesc45_impersonate" {
  ...
  service_account_id = google_service_account.privesc45_org_policy[0].name
