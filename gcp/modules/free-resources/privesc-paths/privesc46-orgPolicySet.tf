# Privesc Path 46: Organization Policy Set
#
...
#
# NOTE: This path is DISABLED by default (enable_privesc46 = false) because
...
#
# Enable with: enable_privesc46 = true (requires gcp_organization_id)

resource "google_service_account" "privesc46_org_policy" {
  count = var.enable_privesc46 ? 1 : 0

  account_id   = "${var.resource_prefix}46-org-policy"
  display_name = "Privesc46 - orgpolicy.policy.set"
...
resource "google_project_iam_member" "privesc46_org_policy" {
  count = var.enable_privesc46 ? 1 : 0

  project = var.project_id
  role    = "roles/orgpolicy.policyAdmin"
  member  = "serviceAccount:${google_service_account.privesc46_org_policy[0].email}"
}
...
resource "google_service_account_iam_member" "privesc46_impersonate" {
  ...
  service_account_id = google_service_account.privesc46_org_policy[0].name
