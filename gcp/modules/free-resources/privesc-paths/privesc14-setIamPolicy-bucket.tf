# Privesc Path 14: setIamPolicy on Storage Bucket
#
...
resource "google_service_account" "privesc14_bucket_iam" {
...
resource "google_project_iam_member" "privesc14_role" {
  project = var.project_id
  role    = google_project_iam_custom_role.bucket_iam.id
  member  = "serviceAccount:${google_service_account.privesc14_bucket_iam.email}"
}

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "privesc14_impersonate" {
  service_account_id = google_service_account.privesc14_bucket_iam.name
