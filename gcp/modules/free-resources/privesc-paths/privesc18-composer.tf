# Privesc Path 18: Cloud Composer (Airflow) Environment
#
...
resource "google_service_account" "privesc18_composer" {
...
resource "google_project_iam_member" "privesc18_role" {
  ...
  member  = "serviceAccount:${google_service_account.privesc18_composer.email}"
}
...
resource "google_service_account_iam_member" "privesc18_impersonate" {
  service_account_id = google_service_account.privesc18_composer.name
