# Privesc Path 21: setIamPolicy on Pub/Sub Resources
#
...
resource "google_service_account" "privesc21_pubsub_iam" {
...
# Assign the role
resource "google_project_iam_member" "privesc21_role" {
  project = var.project_id
  role    = google_project_iam_custom_role.pubsub_iam.id
  member  = "serviceAccount:${google_service_account.privesc21_pubsub_iam.email}"
}

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "privesc21_impersonate" {
  service_account_id = google_service_account.privesc21_pubsub_iam.name
