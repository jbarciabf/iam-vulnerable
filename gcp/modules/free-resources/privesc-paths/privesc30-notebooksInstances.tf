# Privesc Path 30: Vertex AI Notebooks with Privileged SA
#
...
resource "google_service_account" "privesc30_notebooks" {
  account_id   = "${var.resource_prefix}30-notebooks"
  display_name = "Privesc30 - Vertex AI Notebooks"
  description  = "Can escalate via notebooks.instances.create"
  project      = var.project_id
}

# Create a custom role with Notebooks permissions
resource "google_project_iam_custom_role" "privesc30_notebooks" {
  role_id     = "${var.resource_prefix}_30_notebooks"
  title       = "Privesc30 Notebook Instance Creator"
...
resource "google_project_iam_member" "privesc30_notebooks" {
  project = var.project_id
  role    = google_project_iam_custom_role.privesc30_notebooks.id
  member  = "serviceAccount:${google_service_account.privesc30_notebooks.email}"
}
...
resource "google_service_account_iam_member" "privesc30_impersonate" {
  service_account_id = google_service_account.privesc30_notebooks.name
