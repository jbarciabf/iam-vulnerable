# Privesc Path 29: GKE Cluster with Privileged Node SA
#
...
resource "google_service_account" "privesc29_gke" {
...
resource "google_project_iam_custom_role" "privesc29_gke" {
...
resource "google_project_iam_member" "privesc29_gke" {
  project = var.project_id
  role    = google_project_iam_custom_role.privesc29_gke.id
  member  = "serviceAccount:${google_service_account.privesc29_gke.email}"
}
...
resource "google_service_account_iam_member" "privesc29_impersonate" {
  service_account_id = google_service_account.privesc29_gke.name
