# Privesc Path 24: OpenID Token Generation
#
...
resource "google_service_account" "privesc24_get_oidc_token" {
...
resource "google_project_iam_custom_role" "privesc24_oidc_creator" {
...
resource "google_service_account_iam_member" "privesc24_oidc_creator" {
  service_account_id = google_service_account.high_priv.name
  role               = google_project_iam_custom_role.privesc24_oidc_creator.id
  member             = "serviceAccount:${google_service_account.privesc24_get_oidc_token.email}"
}

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "privesc24_impersonate" {
  service_account_id = google_service_account.privesc24_get_oidc_token.name
