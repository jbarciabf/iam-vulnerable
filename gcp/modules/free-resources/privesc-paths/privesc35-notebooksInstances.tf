# Privesc Path 35: Vertex AI Notebooks with Privileged SA
#
...
resource "google_service_account" "privesc35_notebooks" {
  account_id   = "${var.resource_prefix}35-notebooks"
  display_name = "Privesc35 - Vertex AI Notebooks"
...
resource "google_project_iam_custom_role" "privesc35_notebooks" {
  role_id     = "${var.resource_prefix}_35_notebooks"
  title       = "Privesc35 Notebook Instance Creator"
...
resource "google_project_iam_member" "privesc35_notebooks" {
  project = var.project_id
  role    = google_project_iam_custom_role.privesc35_notebooks.id
  member  = "serviceAccount:${google_service_account.privesc35_notebooks.email}"
}

# Grant actAs on the high-privilege SA
resource "google_service_account_iam_member" "privesc35_actas" {
...
resource "google_service_account_iam_member" "privesc35_impersonate" {
  service_account_id = google_service_account.privesc35_notebooks.name
