# Privesc Path 22: Cloud Scheduler with Service Account
#
...
resource "google_service_account" "privesc22_scheduler" {
...
# Assign the role
resource "google_project_iam_member" "privesc22_role" {
  project = var.project_id
  role    = google_project_iam_custom_role.scheduler.id
  member  = "serviceAccount:${google_service_account.privesc22_scheduler.email}"
}
...
resource "google_service_account_iam_member" "privesc22_impersonate" {
  service_account_id = google_service_account.privesc22_scheduler.name
