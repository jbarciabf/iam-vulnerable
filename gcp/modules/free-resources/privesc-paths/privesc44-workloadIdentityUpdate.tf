# Privesc Path 45: Workload Identity Federation Update (Hijack Existing Provider)
#
...
#
# NOTE: This path is DISABLED by default (enable_privesc45 = false)
#       Enable with: enable_privesc45 = true

resource "google_service_account" "privesc45_workload_identity_update" {
  count = var.enable_privesc45 ? 1 : 0

  account_id   = "${var.resource_prefix}45-wid-update"
  display_name = "Privesc45 - Workload Identity Update"
...
resource "google_project_iam_custom_role" "privesc45_workload_identity_update" {
  count = var.enable_privesc45 ? 1 : 0

  role_id     = "${var.resource_prefix}_45_workload_identity_update"
  title       = "Privesc45 Workload Identity Updater"
...
resource "google_project_iam_member" "privesc45_workload_identity_update" {
  count = var.enable_privesc45 ? 1 : 0

  project = var.project_id
  role    = google_project_iam_custom_role.privesc45_workload_identity_update[0].id
  member  = "serviceAccount:${google_service_account.privesc45_workload_identity_update[0].email}"
}

# No actAs binding needed - changes issuer URI, doesn't need actAs

# Allow the attacker to impersonate this service account
resource "google_service_account_iam_member" "privesc45_impersonate" {
  count = var.enable_privesc45 ? 1 : 0

  service_account_id = google_service_account.privesc45_workload_identity_update[0].name
