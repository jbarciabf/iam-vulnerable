# Privesc Path 35: GKE Cluster with Privileged Node SA
#
...
resource "google_service_account" "privesc35_gke" {
  account_id   = "${var.resource_prefix}35-gke"
  display_name = "Privesc35 - GKE Clusters"
...
resource "google_project_iam_custom_role" "privesc35_gke" {
  role_id     = "${var.resource_prefix}_35_gke"
  title       = "Privesc35 GKE Cluster Creator"
...
resource "google_project_iam_member" "privesc35_gke" {
  project = var.project_id
  role    = google_project_iam_custom_role.privesc35_gke.id
  member  = "serviceAccount:${google_service_account.privesc35_gke.email}"
}
...
resource "google_service_account_iam_member" "privesc35_impersonate" {
  service_account_id = google_service_account.privesc35_gke.name
