...
output "privesc_service_accounts" {
  ...
  value = merge(
    ...
    # Conditionally enabled paths
    var.enable_privesc11 ? { "privesc11-setMetadata-compute" = google_service_account.privesc11_set_metadata[0].email } : {},
    var.enable_privesc12 ? { "privesc12-osLogin" = google_service_account.privesc12_os_login[0].email } : {},
    var.enable_privesc13 ? { "privesc13-setServiceAccount" = google_service_account.privesc13_set_sa[0].email } : {},
    var.enable_privesc16 ? { "privesc16-updateFunction" = google_service_account.privesc16_update_function[0].email } : {},
    var.enable_privesc17 ? { "privesc17-sourceCodeSet" = google_service_account.privesc17_source_code_set[0].email } : {},
    var.enable_privesc19 ? { "privesc19-runServicesUpdate" = google_service_account.privesc19_run_update[0].email } : {},
    var.enable_privesc42 ? { "privesc42-orgPolicySet" = google_service_account.privesc42_org_policy[0].email } : {},
  )
}
