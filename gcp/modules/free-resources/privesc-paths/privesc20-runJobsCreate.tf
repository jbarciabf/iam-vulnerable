...
resource "google_project_iam_member" "privesc20_run_jobs" {
  project = var.project_id
  role    = google_project_iam_custom_role.privesc20_run_jobs.id
  member  = "serviceAccount:${google_service_account.privesc20_run_jobs.email}"
}
...
resource "google_service_account_iam_member" "privesc20_impersonate" {
  service_account_id = google_service_account.privesc20_run_jobs.name
